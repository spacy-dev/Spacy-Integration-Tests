...
inline auto product_hilbert_space_1D(int scalar_degrees_of_freedom)
{
    assert(scalar_degrees_of_freedom > 1);
    auto mesh = std::make_shared<dolfin::UnitSquareMesh>(scalar_degrees_of_freedom-1, scalar_degrees_of_freedom-1);
    auto V = std::make_shared<L2Functional::CoefficientSpace_x>(mesh);
    return Spacy::FEniCS::makeHilbertSpace(V,{0,1,2},{});
}

inline auto primal_dual_product_hilbert_space_1D(int scalar_degrees_of_freedom)
{
    assert(scalar_degrees_of_freedom > 1);
    auto mesh = std::make_shared<dolfin::UnitSquareMesh>(scalar_degrees_of_freedom-1, scalar_degrees_of_freedom-1);
    auto V = std::make_shared<L2Functional::CoefficientSpace_x>(mesh);
